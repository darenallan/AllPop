...
window.subscribeMyOrders = function (userId, onUpdate, onError) {
  ...
  if (DEV) console.log("📡 [ORDERS] Subscribing to client orders, userId:", userId);
  ...
  return window.db
    ...
    .onSnapshot(
      (snap) => {
        ...
        if (DEV) console.log(
          "✅ [ORDERS] Client orders updated:",
          orders.length,
          "total",
        );
        orders.forEach((o) => {
          if (DEV) console.log(
            `   Order ${o.id}: ${o.status} (${STATUS_DISPLAY[o.status]?.label || "unknown"})`,
          );
        });
        if (onUpdate) onUpdate(orders);
      },
      (err) => {
        if (DEV) console.error("❌ [ORDERS] Client listener error:", err);
...
window.subscribeVendorOrders = function (shopId, onUpdate, onError) {
  if (!window.db || !shopId) {
    if (DEV) console.error("❌ [ORDERS] Invalid parameters: shopId=", shopId);
    return null;
  }

  if (DEV) console.log("📡 [ORDERS] Subscribing to vendor orders, shopId:", shopId);
  ...
  return window.db
    ...
    .onSnapshot(
      ...
      (err) => {
        if (DEV) console.error("❌ [ORDERS] Vendor listener error:", err);
        if (onError) onError(err);
      },
    );
};
...
// ── SUBSCRIBE À UNE COMMANDE SPÉCIFIQUE (POUR SUIVI DÉTAIL CLIENT)
window.subscribeOrderDetails = function (orderId, onUpdate, onError) {
  if (!window.db || !orderId) {
    if (DEV) console.error("❌ [ORDERS] Invalid parameters for order details");
    return null;
  }

  if (DEV) console.log("📡 [ORDERS] Subscribing to order details:", orderId);

  // Real-time listener sur le document spécifique
  return window.db
    .collection("orders")
    .doc(orderId)
    .onSnapshot(
      (doc) => {
        if (!doc.exists) {
          console.warn("⚠️ [ORDERS] Order document not found:", orderId);
          return;
        }
        const order = { id: doc.id, ...doc.data() };
        if (DEV) console.log("✅ [ORDERS] Order detail updated:", order.status);
        if (onUpdate) onUpdate(order);
      },
      (err) => {
        if (DEV) console.error("❌ [ORDERS] Order details listener error:", err);
        if (onError) onError(err);
      },
    );
};
...
// ── TRANSITION DE STATUT - ADMIN (validate) ────────────────────────
window.validateOrder = async function (orderId) {
  if (!window.db || !orderId) return;

  if (!confirm("Valider cette commande ? Elle sera transférée au vendeur."))
    return;

  try {
    if (DEV) console.log("✅ [ORDERS] Admin validating order:", orderId);
    ...
    if (DEV) console.log("✅ [ORDERS] Order validated, ready for vendor");
    if (window.showToast)
      window.showToast(
        "Commande validée ✓ Le vendeur reçoit l'ordre de préparation.",
        "success",
      );
  } catch (err) {
    if (DEV) console.error("❌ [ORDERS] Error validating order:", err);
...
window.markOrderReady = async function (orderId) {
  ...
  try {
    if (DEV) console.log("📚 [ORDERS] Marking order ready:", orderId);
    ...
    if (DEV) console.log("✅ [ORDERS] Order marked ready");
    if (window.showToast)
      window.showToast(
        "Commande prête ✓ Les livreurs peuvent la prendre en charge.",
        "success",
      );
  } catch (err) {
    if (DEV) console.error("❌ [ORDERS] Error marking ready:", err);
    if (window.showToast) window.showToast("Erreur : " + err.message, "danger");
  }
};
...
window.acceptOrder = async function (orderId) {
  ...
  if (
    !confirm("Accepter cette commande ? Vous deviendrez responsable du colis.")
  )
    return;
  ...
  try {
    ...
    if (DEV) console.log(
      "📦 [ORDERS] Courier accepting order:",
      orderId,
      "| courierId:",
      courier.uid,
    );
    ...
    if (DEV) console.log("✅ [ORDERS] Order accepted, in transit");
    if (window.showToast)
      window.showToast("Commande acceptée ✓ Le client est notifié.", "success");
  } catch (err) {
    if (DEV) console.error("❌ [ORDERS] Error accepting order:", err);
    if (window.showToast) window.showToast("Erreur : " + err.message, "danger");
  }
};
...
window.markOrderDelivered = async function (orderId) {
  ...
  try {
    ...
    if (DEV) console.log("📚 [ORDERS] Courier confirming delivery:", orderId);
    ...
    if (DEV) console.log("✅ [ORDERS] Order delivered");
    if (window.showToast) window.showToast("Livraison confirmée ✓", "success");
  } catch (err) {
    if (DEV) console.error("❌ [ORDERS] Error marking delivered:", err);
    if (window.showToast) window.showToast("Erreur : " + err.message, "danger");
  }
};
...
window.cancelOrder = async function (orderId, reason = "") {
  ...
  try {
    if (DEV) console.log("❌ [ORDERS] Cancelling order:", orderId);
    ...
    if (DEV) console.log("✅ [ORDERS] Order cancelled");
    if (window.showToast) window.showToast("Commande annulée.", "success");
  } catch (err) {
    if (DEV) console.error("❌ [ORDERS] Error cancelling:", err);
    if (window.showToast) window.showToast("Erreur : " + err.message, "danger");
  }
};
...
window.migrateOrderStatuses = async function () {
  if (!window.db || !firebase.auth().currentUser) {
    if (DEV) console.error("❌ [MIGRATION] Not authenticated");
    return;
  }
  ...
  if (
    !confirm(
      "⚠️ ADMIN ONLY ⚠️\n\nMigrer TOUS les statuts ? Cette action est irréversible.",
    )
  )
    return;

  try {
    if (DEV) console.log("🔄 [MIGRATION] Starting order status migration...");
    ...
    snap.docs.forEach((doc) => {
      ...
      if (newStatus) {
        if (DEV) console.log(`   Migrating ${doc.id}: ${oldStatus} → ${newStatus}`);
    ...
    if (count === 0) {
      if (DEV) console.log("✅ [MIGRATION] No orders to migrate");
      if (window.showToast)
        window.showToast("Aucune commande à migrer.", "info");
      return;
    }

    await batch.commit();
    if (DEV) console.log(`✅ [MIGRATION] Successfully migrated ${count} orders`);
    if (window.showToast)
      window.showToast(`Migré ${count} commandes avec succès.`, "success");
  } catch (err) {
    if (DEV) console.error("❌ [MIGRATION] Failed:", err);
    if (window.showToast)
      window.showToast("Erreur migration : " + err.message, "danger");
  }
};
...
if (DEV) console.log("✅ [ORDERS] Module loaded");
