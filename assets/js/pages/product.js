...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // --- NOUVELLE LOGIQUE DE RÉCUPÉRATION PAR SLUG-ID ---
  const path = window.location.pathname;
  // On récupère le dernier segment de l'URL (le slug)
  let slug = path.split("/").filter(Boolean).pop();
  // Fallback: on regarde aussi les paramètres GET pour l'ID
  ...
  if (slug && slug.includes("--")) {
    ...
    idFromSlug = parts[parts.length - 1]; // L'ID est après le dernier --
  }

  // Fusion des sources d'ID (priorité à idFromSlug, puis idFromQuery)
  const productId = idFromSlug || idFromQuery;
  ...
  (async function () {
    try {
      ...
      // D'abord on essaye de récupérer par ID direct (plus fiable et rapide)
      if (productId) {
        try {
          var snap = await db.collection("products").doc(productId).get();
          if (snap.exists) {
            pid = snap.id;
            product = Object.assign({ id: pid }, snap.data());
          }
        } catch (e) {}
      }

      // Fallback: si pas trouvé et qu'on a un slug valide, chercher par le champ slug
      if (!product && slug && slug !== "product") {
        // On récupère le slug sans l'ID (avant les --)
        let slugOnly = slug;
        if (slug.includes("--")) {
      ...
      if (shopId !== "unknown") {
        try {
          var s = await db.collection("shops").doc(shopId).get();
  ...
  function renderProduit(product, shopId, shopData, reviews) {
    ...
    // 1. Titre de l'onglet (Ex: "iPhone 15 | Sanhia")
    document.title = product.name + " | Sanhia Marketplace";
    ...
    var mqHTML = [...mqData, ...mqData]
      .map(function (i) {
        return (
          '<div class="ax-mqi">' + i.k + " <strong>" + i.v + "</strong></div>"
        );
      })
      .join("");
    var dspecs = mqData
      .map(function (i) {
        return (
          '<div class="ax-dsrow"><span class="ax-dsk">' +
          i.k +
          '</span><span class="ax-dsv">' +
          i.v +
          "</span></div>"
        );
      })
    ...
    var colorsHTML =
      product.colors && product.colors.length
        ? '<div class="rv rv3"><p class="ax-varlabel">Couleur</p><div class="ax-colors">' +
          product.colors
            .map(function (c, i) {
              return (
                '<div class="ax-csw' +
    ...
    var sizesHTML =
      product.sizes && product.sizes.length
        ? '<div class="rv rv3"><p class="ax-varlabel">Taille</p><div class="ax-sizes">' +
          product.sizes
            .map(function (s, i) {
              return (
                '<button class="ax-spill' +
                (i === 0 ? " on" : "") +
                '" data-vt="size" data-val="' +
                s +
                "\" onclick=\"axVar('size','" +
                s +
                "',this)\">" +
    ...
    root.innerHTML =
      ...
      (imgs.length > 1
        ? '<div class="ax-thumbs">' +
          ...
          "</div>"
        : "") +
      '<div class="ax-gal-num"><strong>01</strong>/ ' +
      String(imgs.length).padStart(2, "0") +
      "</div>" +
      "</div>" +
      '<div class="ax-info">' +
      '<p class="ax-eyebrow rv">' +
      (product.category || "Collection Premium") +
      "</p>" +
      '<h1 class="ax-pname rv rv1">' +
      product.name +
      "</h1>" +
      '<div class="ax-rrow rv rv2"><span class="ax-stars">' +
      starH(avg) +
      '</span><span class="ax-rmeta">' +
      avg +
      '/5 &nbsp;·&nbsp; <a href="#ax-tabs">' +
      reviews.length +
      " avis</a></span></div>" +
      '<div class="ax-pblock rv rv2"><p class="ax-plabel">Prix</p><div class="ax-pprice">' +
      fmt(product.price) +
      "<small>FCFA</small></div>" +
      (hd
        ? '<div class="ax-pstrike">' +
          fmt(product.originalPrice) +
          ' FCFA <span class="ax-save">−' +
          disc +
          "%</span></div>"
        : "") +
      "</div>" +
      colorsHTML +
      sizesHTML +
      '<div class="ax-sline rv rv3"><div class="ax-sdot ' +
      sc +
      ...
      '<div class="ax-qty"><button class="ax-qbtn" onclick="axQty(-1)">−</button><input type="number" class="ax-qnum" id="ax-qty" value="1" min="1" max="' +
      (stk || 999) +
      '"><button class="ax-qbtn" onclick="axQty(1)">+</button></div>' +
      ...
      "</div>" +
      '<div class="ax-seller rv"><div class="ax-sava">' +
      savaHTML +
      '</div><div><div class="ax-sname">Vendu par ' +
      (shopData.name || "Boutique Sanhia") +
      '</div><div class="ax-ssub"><span style="color:var(--gold)">' +
      starH(shopData.rating || 0) +
      "</span><span>📍 " +
      ...
      "</div>" +
      "</section>" +
      '<div class="ax-mq"><div class="ax-mqt">' +
      mqHTML +
      "</div></div>" +
      '<section class="ax-tabs" id="ax-tabs"><div class="ax-ti">' +
      '<div class="ax-tnav rv"><button class="ax-tbtn on" onclick="axTab(\'desc\',this)">Description</button><button class="ax-tbtn" onclick="axTab(\'rev\',this)">Avis clients (' +
      reviews.length +
      ")</button></div>" +
      '<div class="ax-tpanel on" id="ax-p-desc"><div class="ax-desc-grid rv"><p class="ax-dtxt">' +
      (
        product.description || '<em style="opacity:.4">Aucune description.</em>'
      ).replace(/\n/g, "<br>") +
      "</p>" +
      (dspecs ? '<div class="ax-dspecs">' + dspecs + "</div>" : "") +
      "</div></div>" +
      '<div class="ax-tpanel" id="ax-p-rev"><div class="ax-rl"><div class="ax-rs rv"><div class="ax-avghuge">' +
      avg +
      '</div><div class="ax-avgstars">' +
      starH(avg) +
      ...
      (reviews.length
        ? reviews
            .map(function (r) {
              return (
                '<div class="ax-rcard"><div class="ax-rchead"><div class="ax-rcwho"><div class="ax-rcava">' +
                (r.userName || "A").charAt(0).toUpperCase() +
                '</div><div><div class="ax-rcname">' +
                (r.userName || "Anonyme") +
                '</div><div class="ax-rcdate">' +
                (r.createdAt
                  ? new Date(r.createdAt.seconds * 1000).toLocaleDateString(
                      "fr-FR",
                    )
                  : "") +
                '</div></div></div><div class="ax-rcstars">' +
                starH(r.rating || 5) +
                '</div></div><p class="ax-rctxt">' +
                (r.comment || "") +
                "</p></div>"
              );
            })
  ...
  window.axImg = function (idx) {
    var m = document.getElementById("ax-main-img"),
    ...
    if (n)
      n.innerHTML =
        ...
        "</strong>/ " +
        String(window.currentImages.length).padStart(2, "0");
    document.querySelectorAll(".ax-t").forEach(function (t, i) {
  ...
  window.axCart = function () {
    try {
      if (!window.currentProduct) {
        if (window.showToast)
          window.showToast("Produit indisponible", "danger");
        return;
      }
      var qty = Math.max(
        1,
      ...
      if (typeof addToCart === "function") {
        ...
        var ei = cart.findIndex(function (c) {
          return (
            c.id === item.id &&
      ...
      console.error(err);
      if (window.showToast)
        window.showToast("Impossible d'ajouter au panier", "danger");
    }
  };
  window.axWish = function () {
    ...
    if (b && !b.querySelector("svg")) {
      b.innerHTML =
        "<span style=\"font-family:'Unbounded',sans-serif; font-weight:700; font-size:11px; letter-spacing:.15em; text-transform:uppercase;\">FAV</span>";
    }
  ...
  window.axTab = function (n, btn) {
    document.querySelectorAll(".ax-tbtn").forEach(function (b) {
      b.classList.remove("on");
    });
    document.querySelectorAll(".ax-tpanel").forEach(function (p) {
  ...
  window.axHS = function (r) {
    document.querySelectorAll("#ax-spick span").forEach(function (s, i) {
      s.style.color = i < r ? "var(--gold)" : "rgba(11,10,8,.18)";
    });
  };
  window.axSS = function (r) {
    var el = document.getElementById("ax-rrating");
    if (el) el.value = r;
    window.axHS(r);
  };
  window.axReview = async function (e) {
    ...
    if (!name) {
      if (window.showToast) window.showToast("⚠ Entrez votre nom", "danger");
      return;
    }
    if (!comment) {
      if (window.showToast) window.showToast("⚠ Entrez votre avis", "danger");
      return;
    }
    var user = firebase.auth().currentUser;
    if (!user) {
      if (window.showToast) window.showToast("⚠ Connexion requise", "danger");
      return;
    }
    try {
      btn.disabled = true;
  ...
  window.axContactSeller = async function () {
    ...
    if (typeof window.openSellerChat !== "function") {
      if (window.showToast)
        window.showToast("Messagerie indisponible.", "danger");
      return;
    }
    try {
      var shop = window.currentShopData || {},
        p = window.currentProduct || {};
      await window.openSellerChat({
        shopId: window.currentShopId || p.shopId || "",
        sellerId:
          shop.ownerId || shop.sellerId || p.sellerId || p.ownerId || "",
    ...
    try {
      var q = product.category
        ? db
            ...
            .limit(9)
        : db.collection("products").limit(9);
      var snap3 = await q.get();
      var items = [];
      snap3.forEach(function (d) {
        if (d.id !== product.id)
      ...
      if (!items.length) {
        relGrid.innerHTML =
          '<div style="grid-column:1/-1;text-align:center;padding:60px;color:var(--smoke);font-size:11px">Aucun produit similaire</div>';
        return;
      }
      relGrid.innerHTML = items
        .map(function (p2) {
          var img2 =
            (Array.isArray(p2.images) ? p2.images[0] : null) ||
            p2.image ||
            "assets/img/placeholder-product-1.svg";
          var hd2 = p2.originalPrice && p2.originalPrice > p2.price;
          var d2 = hd2
            ? Math.round(
                ((p2.originalPrice - p2.price) / p2.originalPrice) * 100,
              )
            : 0;
          ...
          return (
            ...
            (hd2 ? '<div class="ax-rdisc">−' + d2 + "%</div>" : "") +
            '<img src="' +
            img2 +
            '" alt="' +
            p2.name +
            '" onerror="this.src=\'assets/img/placeholder-product-1.svg\'">' +
            '<div class="ax-roverlay"><button class="ax-rquick" onclick="event.stopPropagation();axQuick(\'' +
            p2.id +
            "')\">+ Ajouter au panier</button></div></div>" +
            '<div class="ax-rbody"><p class="ax-rcat">' +
            (p2.category || "Produit") +
            "</p>" +
            '<p class="ax-rname">' +
            p2.name +
            "</p>" +
            '<div class="ax-rfoot"><span class="ax-rprice">' +
            fmt(p2.price) +
            " <small>FCFA</small></span>" +
            '<span class="ax-rstars">' +
            starH(p2.rating || 0) +
            "</span></div></div>" +
            "</div>"
          );
      ...
      console.warn("[produit] loadRelated:", err);
    }
  }
