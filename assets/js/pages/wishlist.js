...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function buildFilters() {
    ...
    filtersEl.innerHTML =
      ...
      cats
        .map(function (c) {
          return (
            '<button class="wl-filter" data-cat="' + c + '">' + c + "</button>"
          );
        })
        .join("");
    filtersEl.querySelectorAll(".wl-filter").forEach(function (btn) {
      btn.addEventListener("click", function () {
        filtersEl.querySelectorAll(".wl-filter").forEach(function (b) {
  ...
  function renderWl() {
    ...
    products.forEach(function (p, idx) {
      ...
      var op = Number(
        String(p.originalPrice || 0).replace(/[^\d.-]/g, "") || 0,
      );
      var hasD = op > price && op > 0;
      var stock = parseInt(p.stock || 0, 10);
      var stLbl =
        stock === 0 ? "Épuisé" : stock <= 5 ? "Plus que " + stock : "En stock";
      ...
      card.innerHTML =
        '<a href="' +
        pUrl +
        '" class="wl-card-img-wrap">' +
        '<img src="' +
        img +
        '" class="wl-card-img" alt="' +
        (p.name || "") +
        '" onerror="this.src=\'assets/img/placeholder-product-1.svg\'">' +
        (hasD
          ? '<div class="wl-card-badge sale">-' +
            Math.round((1 - price / op) * 100) +
            "%</div>"
          : "") +
        '<button class="wl-card-remove" onclick="wlRemove(\'' +
        p.id +
        '\',event)" title="Retirer des favoris"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/></svg></button>' +
        '<div class="wl-card-overlay"><button class="wl-card-add-btn" onclick="wlAddToCart(\'' +
        p.id +
        "',event)\" " +
        (stock === 0 ? "disabled" : "") +
        "" +
        (stock === 0 ? ' style="opacity:.4;pointer-events:none"' : "") +
        ">+ " +
        ...
        (p.category
          ? '<span class="wl-card-cat">' + p.category + "</span>"
          : "") +
        '<a href="' +
        pUrl +
        '" class="wl-card-name">' +
        ...
        "</a>" +
        (p.shopName
          ? '<span class="wl-card-shop">' + p.shopName + "</span>"
          : "") +
        '<div class="wl-card-footer"><div><div class="wl-card-price">' +
        wlFmt(price) +
        "</div>" +
        (hasD ? '<div class="wl-card-original">' + wlFmt(op) + "</div>" : "") +
        '</div><span class="wl-card-stock ' +
        stCls +
        '">' +
        stLbl +
        "</span></div>" +
    ...
    if (!products.length && allProducts.length) {
      gridEl.innerHTML =
        '<p style="padding:40px;text-align:center;color:var(--smoke)">Aucun produit dans cette catégorie.</p>';
    }
  }
  ...
  async function loadWishlistFirestore() {
    var ids = [];
    try {
      var rawItems = JSON.parse(
        localStorage.getItem("ac_wishlist") || "[]",
      ).filter(Boolean);
      // LE FILTRE MAGIQUE : Si c'est un objet on prend son .id, sinon on le garde tel quel
      ids = rawItems
        .map(function (item) {
          return typeof item === "object" && item !== null ? item.id : item;
        })
        .filter(Boolean); // On revérifie pour enlever les vides
    } catch {
      ids = [];
    }
    if (!ids.length) {
    ...
    try {
      ...
      for (var chunk of chunks) {
        ...
        snap.docs.forEach(function (d) {
          products.push(Object.assign({ id: d.id }, d.data()));
        });
      }
  ...
  window.wlAddToCart = function (id, e) {
    ...
    var p = allProducts.find(function (p) {
      return p.id === id;
    });
    if (!p) return;
    var existing = cart.find(function (i) {
      return i.pid === id || i.id === id;
    });
    if (existing) {
    ...
    if (window.showToast)
      window.showToast((p.name || "Produit") + " ajouté au panier", "success");
    if (window.updateCartBadge) window.updateCartBadge();
  };
