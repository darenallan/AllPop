...
async function initializeCatalogue() {
  ...
  const categoriesPromise = window.db.collection('categories').get();

  // 2. Charger TOUTES les boutiques en une seule fois (Optimisation majeure)
  const shopsPromise = window.db.collection('shops').get();

  // 3. Charger les produits
  const productsPromise = window.db.collectionGroup('products').get();
